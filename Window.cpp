...
Window::Window(int w, int h) :width(w), height(h)
{
	...
	bmpInfo.bmiHeader.biWidth = newWidth;         //指定图象的宽度，单位是像素
	bmpInfo.bmiHeader.biHeight = newHeight;       //指定图象的高度，单位是像素
	...
	memset(bmpBuffer, 0, newWidth * newHeight * 4);//清空bmpBuffer为0，32位位图每个像素占4个字节
